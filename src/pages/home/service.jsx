...
const Service = () => {
    ...
    return (
        <section className='bg-secondary-600 h-[1798px] md:h-[895px] lg:h-[895px] xl:h-[895px] 2xl:h-[895px] flex flex-col md:justify-center lg:justify-center xl:justify-center 2xl:justify-center justify-start items-center gap-[80px] relative main-bg '> 
                ...
                <div className='grid grid-cols-1 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-3 2xl:grid-cols-3 md:gap-[50px] lg:gap-[50px] xl:gap-[50px] 2xl:gap-[50px] gap-[40px] md:w-[1140px] lg:w-[1140px] xl:w-[1140px] 2xl:w-[1140px] w-auto h-[377px]'>
                    {
                        cardDetails.map((cD, i) =>
                            <div onMouseOver={() => setNumber(i)} key={i} className={`${i === number ? "shadow-2xl" : ""} flex flex-col justify-center items-center gap-[15px] md:w-[347px] lg:w-[347px] xl:w-[347px] 2xl:w-[347px] w-[302px] h-[400px] md:h-[377px] lg:h-[377px] xl:h-[377px] 2xl:h-[377px] card-z-index bg-white-white rounded-md hover:shadow-2xl cursor-pointer active:shadow-xl  `}>
                                <div>
                                    <Image src={cD.icon} width={90} height={85} alt="card-icon" />
                                </div>
                                <h1 className='font-semibold text-[26px] text-center px-14 md:px-0 lg:px-0 xl:px-0 2xl:px-0'>{cD.title}</h1>
                                <p className='font-normal text-base  text-center w-[296px] px-4 md:px-0 lg:px-0 xl:px-0 2xl:px-0'>{cD.desc}</p>
                            </div>
                        )
                    }
                </div> 
